...
class TemporalContrastiveLoss(nn.Module):
    def __init__(self,temporal_encoder, num_features=16, weight_time=0.5, weight_core=0.5, weight_adj=0.1):
        super().__init__()
    ...
    def forward(self, z, query_idx, neighbor_idx, edge_times, current_time, t_s, t_e, G):
        ...
        if timestamps_window.numel() == 0:
            # 如果时间窗口内没有边，损失为0
            loss_time = torch.tensor(0.0, device=z.device)
        else:
            # 对时间窗口内的时间戳进行编码
            ...
            if neighbor_edge_times.numel() == 0:
                ...
                # 计算两个时间编码的平均值
                mean_phi_t_window = phi_t_window.mean(dim=0)  # [num_features]
                mean_phi_t_neighbors = phi_t_neighbors.mean(dim=0)  # [num_features]

                # 对时间编码进行归一化
                phi_t_window = F.normalize(phi_t_window, dim=0)
                phi_t_neighbors = F.normalize(phi_t_neighbors, dim=0)
        ...
        # 计算核心度的均方误差损失
        loss_core = F.mse_loss(neighbor_cores.float(), query_core.float().expand_as(neighbor_cores))

        # ==================== Loss 3: 邻接矩阵重构损失 ====================
        # TODO: 邻接矩阵重构损失 暂未使用
        num_nodes = z.size(0)
        H = z  # [num_nodes, embedding_dim]

        # 构建邻接矩阵 A
        A = torch.zeros((num_nodes, num_nodes), device=z.device)
        ...
        # 计算 A_hat = H H^T
        A_hat = torch.mm(H, H.t())

        # 计算邻接矩阵重构损失
        loss_adj = F.mse_loss(A_hat, A)

        # ==================== 总损失 ====================
        total_loss = self.weight_time * loss_time + self.weight_core * loss_core 
        # self.weight_adj * loss_adj
        return total_loss